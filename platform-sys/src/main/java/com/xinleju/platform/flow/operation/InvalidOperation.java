	...
	protected void sendInvalidMessage(List<UserDto> currentApprovers,InstanceUnit instanceUnit) throws Exception{
		...
		for (UserDto userDto:currentApprovers) {
			if(!userDto.getId().equals(user.getId())){
				...
				instanceAccessible.setType("3");//传阅类型的可阅人
				instanceAccessible.setAccessibleId(userDto.getId());
				instanceAccessible.setAccessibleName(userDto.getName());
				instanceAccessible.setDelflag(false);
				instanceAccessible.setConcurrencyVersion(0);
				accessibleList.add(instanceAccessible);
			}
		}
		service.getInstanceAccessibleService().saveBatch(accessibleList);
