...
//			newMessage(instanceUnit, task, currentAc.getApprovalTypeId());
		...
		int xbCount=0;
		Timestamp xbStartDate=null;
		for(int i=0; i<currentApproverList.size(); i++) {
			ApproverUnit approverUnit=currentApproverList.get(i);
			if("XB".equals(approverUnit.getTask().getTaskResult()) && currentApprover.getApproverId().equals(approverUnit.getApproverId())){
				xbCount++;
				if(xbCount==1){
					xbStartDate=approverUnit.getTask().getStartTime();
				}
			}
		}
		if(xbCount>0){
			newCurrentApprover.getTask().setXbStartDate(xbStartDate==null?currentApprover.getTask().getStartTime():xbStartDate);
		}else{
			newCurrentApprover.getTask().setXbStartDate(currentApprover.getTask().getStartTime());
		}
