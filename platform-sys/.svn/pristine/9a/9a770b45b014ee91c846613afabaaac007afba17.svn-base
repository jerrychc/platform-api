...
public class AgentServiceImpl extends BaseServiceImpl<String, Agent> implements
	...
	public Agent queryAgentBy(String userId, String postId, String flCode) {
		...
		// 判断人员有无岗位:false无，true有
		boolean isPost = false;
		if(StringUtils.isNotBlank(postId)){
			isPost = postId.indexOf(".") > 0 ? false : true;
		}
		// 2.1、判断授权范围是否为“仅人员”：此时若流程环节中人员有岗位，则此代理无效；人员无岗位则此代理有效
		if("0".equals(agent.getPostScope())){
			if(isPost){
				return null;
			}
		}else{
			// 2.2.1 若授权范围为"指定岗位"或"全部岗位"：此时若流程环节中人员有岗位，则此代理有效；人员无岗位则此代理无效
			if(isPost){
				// 2.2.2、取得岗位范围并判断指定岗位是否在范围内
