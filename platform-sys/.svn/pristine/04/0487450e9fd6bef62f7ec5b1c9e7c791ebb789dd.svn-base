...
public class PUSupplierServiceImpl implements PUSupplierService {
	...
	public FlowResult<String> saveSupplierInfoDTO(SupplierInfoDTO supplierInfoDTO) {
		...
		if (!list.isEmpty() && list.size() > 0) {
			String sql = "update pt_sys_base_supplier set name='" + name + "', code='" + code + "', STATUS='" + status + "', company_id='" + companyId + "', province_id='" + provinceId + "',city_id='" + cityId + "', representative='" + representative + "', address='" + address + "', work_phone='" + work_phone
					+ "',registration_date='" + registration_date + "',license='" + license + "',phone='" + phone + "',relation_person='" + relation_person + "',remark='" + remark + "',delflag='"+supplierInfoDTO.getIsDelete()+"' where id='" + id+"'";
			jdbcTemplate.update(sql);
		}
		else {
			String sql = "insert into pt_sys_base_supplier " + "(id,name,code,STATUS,company_id,province_id,city_id,representative," + "address,work_phone,registration_date,license,phone,relation_person,remark,delflag) " + "VALUES " + "('" + id + "','" + name + "','" + code + "','" + status + "','"
					+ companyId + "','" + provinceId + "','" + cityId + "','" + representative + "','" + address + "','" + work_phone + "','" + registration_date + "','" + license + "','" + phone + "','" + relation_person + "','" + remark + "','0')";
		   jdbcTemplate.update(sql);
		   
		   String  sqlString2="insert into  pt_sys_base_supplier_company"+"(id,company_id,supper_id,delflag)"
		   + "VALUES"+"('"+IDGenerator.getUUID()+"','"+supplierInfoDTO.getCompanyIds()+"','"+id+"','0')";
		   jdbcTemplate.update(sqlString2);
		}
		result.setResult(id);
		return result;
	}
	...
	public FlowResult<List<String>> saveSupplierAccountDTO(List<SupplierAccountDTO> supplierAccountDTODTOList) {
		...
		for (SupplierAccountDTO supplierAccountDTO : supplierAccountDTODTOList) {
			List<SupplierAccountDTO> list=new ArrayList<SupplierAccountDTO>();
//			String id = IDGenerator.getUUID();
			...
			if(StringUtils.isBlank(supplierAccountDTO.getId())){
				supplierAccountDTO.setId(IDGenerator.getUUID());
			}else {
				sqlString="select * from  pt_sys_base_supplier_accont where id='"+supplierAccountDTO.getId()+"'";
				RowMapper<SupplierAccountDTO> rowMapper = new BeanPropertyRowMapper<SupplierAccountDTO>(SupplierAccountDTO.class);
				list = jdbcTemplate.query(sqlString, rowMapper);
			}
			if (!list.isEmpty() && list.size() > 0) {
				idList.add(supplierAccountDTO.getId());
				String sql="update  pt_sys_base_supplier_accont set  supplier_id='"+supplierId+"', bank_name='"+bankName+"',bank_code='"+bankCode+"',province_id='"+provinceId+"',city_id='"+cityId+"',address='"+address+"',delflag='"+supplierAccountDTO.getIsDelete()+ "' where id='"+supplierAccountDTO.getId()+"'";
				jdbcTemplate.update(sql);
           }else{
			String sql = "insert into pt_sys_base_supplier_accont " + "(id,supplier_id,bank_name,bank_code,province_id,city_id,address,delflag) " + "VALUES " + "('" + supplierAccountDTO.getId() + "','" + supplierId + "','" + bankName + "','" + bankCode + "','" + provinceId + "','" + cityId + "','" + address + "','0')";
