...
//			info.setMsg("管理员执行流程作废成功！");
	...
	public String cancelPassAndRead(String userInfo, String id) {
		DubboServiceResultInfo info = new DubboServiceResultInfo();
		try {
			info = instanceService.cancelPassAndRead(id);
		} catch(Exception e) {
			info.setSucess(false);
...
@Override
	public String getUserPostNew(String userInfo,String params) {
		DubboServiceResultInfo info=new DubboServiceResultInfo();
		try {
			Map<String,Object> param = JacksonUtils.fromJson(params, Map.class);
			OrgnazationPostUserVo scanResult = postUserService.getUserPostNew(param);
			info.setResult(JacksonUtils.toJson(scanResult));
			info.setSucess(true);
			info.setMsg("获取对象成功!");
		} catch (Exception e) {
