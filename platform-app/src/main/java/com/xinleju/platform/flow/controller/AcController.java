...
public class AcController {
	...
	private static Logger log = Logger.getLogger(AcController.class);
	
	@Autowired
	private AcDtoServiceCustomer acDtoServiceCustomer;
	/**
	 * 根据Id获取业务对象
	 * 
	 * @param id  业务对象主键
	...
	public @ResponseBody MessageResult save(@RequestBody AcDto t){
		MessageResult result=new MessageResult();
		try {
			SecurityUserBeanInfo securityUserBeanInfo = LoginUtils.getSecurityUserBeanInfo();
			String userInfo = JacksonUtils.toJson(securityUserBeanInfo);
	...
	public @ResponseBody MessageResult deleteBatch(@PathVariable("ids")  String ids){
		...
		try {
			...
			String dubboResultInfo=acDtoServiceCustomer.deleteAllObjectByIds(userInfo, "{\"id\":\""+ids+"\"}");
			DubboServiceResultInfo dubboServiceResultInfo= JacksonUtils.fromJson(dubboResultInfo, DubboServiceResultInfo.class);
			if(dubboServiceResultInfo.isSucess()){
				String resultInfo= dubboServiceResultInfo.getResult();
				AcDto acDto=JacksonUtils.fromJson(resultInfo, AcDto.class);
	 ...
	 * @return
	 */
	@RequestMapping(value="/queryModifyAcListByPage",method={RequestMethod.POST}, consumes="application/json")
	public @ResponseBody MessageResult queryModifyAcListByPage(@RequestBody Map<String,Object> map){
		MessageResult result=new MessageResult();
		String paramaterJson = JacksonUtils.toJson(map);
		try {
			SecurityUserBeanInfo securityUserBeanInfo = LoginUtils.getSecurityUserBeanInfo();
			...
			String dubboResultInfo=acDtoServiceCustomer.queryModifyAcListByPage(userInfo, paramaterJson);
			DubboServiceResultInfo dubboServiceResultInfo= JacksonUtils.fromJson(dubboResultInfo, DubboServiceResultInfo.class);
			if(dubboServiceResultInfo.isSucess()){
				String resultInfo= dubboServiceResultInfo.getResult();
				PageBeanInfo pageInfo=JacksonUtils.fromJson(resultInfo, PageBeanInfo.class);
	...
	/**
	 * 批量更新节点属性
	 * @param updateAttrData
	 * @return
	 */
	@RequestMapping(value="/batchUpdateAcAttr",method=RequestMethod.POST, consumes="application/json")
	public @ResponseBody MessageResult batchUpdateAcAttr(@RequestBody Map<String,Object> updateAttrData){
		MessageResult result=new MessageResult();
		try {
			SecurityUserBeanInfo securityUserBeanInfo = LoginUtils.getSecurityUserBeanInfo();
			String userInfo = JacksonUtils.toJson(securityUserBeanInfo);

			String saveJson= JacksonUtils.toJson(updateAttrData);
			String dubboResultInfo=acDtoServiceCustomer.batchUpdateAcAttr(userInfo, saveJson);
			DubboServiceResultInfo dubboServiceResultInfo= JacksonUtils.fromJson(dubboResultInfo, DubboServiceResultInfo.class);
			if(dubboServiceResultInfo.isSucess()){
				String resultInfo= dubboServiceResultInfo.getResult();
				AcDto acDto=JacksonUtils.fromJson(resultInfo, AcDto.class);
			...
			try {
				////e.printStackTrace();
				ObjectMapper mapper = new ObjectMapper();
				String  paramJson = mapper.writeValueAsString(updateAttrData);
				log.error("调用batchUpdateAcAttr方法:  【参数"+paramJson+"】======"+"【"+e.getMessage()+"】");
				result.setSuccess(MessageInfo.SAVEERROR.isResult());
				result.setMsg(MessageInfo.SAVEERROR.getMsg()+"【"+e.getMessage()+"】");
			} catch (JsonProcessingException e1) {
				// TODO Auto-generated catch block
