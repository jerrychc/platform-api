...
function setFormData(dataObj){
	...
	$("input[name='postScope'][value='"+dataObj.postScope+"']").click();
...
function doSaveFormAction(){
	...
	var resultData = validateFormData(agentObjDto);
 ...
 * 验证表单数据的处理方法
 */
function validateFormData(agentObjDto){

	var info = '';

	$.ajax({
		url: hostUrl+"flow/agent/queryAgentByUserAndTimeIntersection",
		data: JSON.stringify(agentObjDto),
		type: 'POST',
		contentType: 'application/json',
		async: false,
