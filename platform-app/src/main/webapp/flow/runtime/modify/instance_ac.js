...
$(function () {
    //业务变量
    console.log(window.parent.variableDtoList);
    var variableDtoList = window.parent.variableDtoList;
    //选择器回调函数
    var selectorCallback = function (selectedData, ele) {
        ...
        if (selectedData && $.isArray(selectedData)) {
            ...
            var deptId = variableDtoList.flow_business_dept_id;
            var companyId = variableDtoList.flow_business_company_id;
            var param = {};
            param.deptId = deptId;
            ...
            for (var i = 0; i < selectedData.length; i++) {
                ...
                if (type == '1') {//人员
                    ...
                    if (obj.postName && obj.postName != '') {
                        ...
                        name = obj.name;//obj.prefixName + '/' + obj.name;
                        /*obj.postId = obj.belongOrgId + "." +obj.userId;
                        obj.postName = obj.belongOrgName + "/";*/
                        param.userId = obj.userId;
                        $.ajax({
                            url : hostUrl + "flow/instance/getUserPostNew",
                            ...
                            success : function(data) {
                                if(data.result){
                                    obj.postId = data.result.postId;
                                    obj.postName = data.result.postPrefixName;
                                }else{
                                    obj.postId = obj.belongOrgId + "." +obj.userId;
                                    obj.postName = obj.belongOrgName + "/";
                                }
                ...
                if (i == 0) {
                    ...
                    oldTrObj.find(':input[name="postName"]').val( obj.postName);
                } else {
                    ...
                    selRowObj.find(':input[name="postName"]').val(obj.postName);
                }
            }
        }
    ...
    var initPartnerList = function () {
        ...
        for (var i = (approvePartnerArr.length-1); i > -1; i--) {
    ...
    window.saveAcFormData = function(){
        ...
        if(acBaseInfo.acType=='2'){
            ...
            for (var i = 0; i < originApprovePartnerArr.length; i++) {
                ...
                if((!originApprover.taskStatus||originApprover.taskStatus=='1'||originApprover.taskStatus=='2')&&originApprover.postStatus!='3'){
                    var newApprover = newApproverMap[originApprover.id];
                    if(!newApprover){
                        originApprovePartnerArr.splice(i,1);
                    }else{
                        if(originApprover.participantId!=newApprover.participantId){
                            newApprover.taskStatus=originApprover.taskStatus;
                            newApprover.status=originApprover.status;
                            newApprover.postStatus=originApprover.postStatus;
                            originApprovePartnerArr.splice(i,1);
                        }else{
            ...
            for(var item in newApproverMap){
                ...
                newApprover.taskStatus = newApprover.taskStatus?newApprover.taskStatus:acStatus;
                newApprover.status = newApprover.status?newApprover.status:acStatus;
                newApprover.postStatus = newApprover.postStatus?newApprover.postStatus:acStatus;
