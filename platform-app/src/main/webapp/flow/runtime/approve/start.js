...
//	cachePerson = backData;
    var variableDtoList = resultData.variableDtoList;
                ...
                if(postId == ""){
                    ...
                    $.ajax({
                        ...
                        success : function(data) {
                           /* postId = item.belongOrgId+"."+item.userId;
                            postName = item.belongOrgName + '/';*/
                           if(data.result){
                               postId = data.result.postId;
                               postName = data.result.postPrefixName;
                               item.postId = postId;
                               item.postName = data.result.postName;
                               item.prefixName = data.result.postPrefixName;
                           }else{
                               postId = item.belongOrgId+"."+item.userId;
                               postName = item.belongOrgName + '/';
						   }
	...
	if(backData.length > 0) {
		...
		$.each(backData, function(index, item) {
			if(index>=50){
				$.xljUtils.tip('blue','选择人员不能超过50人!');
				return;
			}
			var postId = item.postId;
            // 			var postName = item.prefixName + '/' + item.postName;
            var postName = item.prefixName;
            if(postId==""){
                postName = item.belongOrgName + "/";
            }
			if(index == 0) {
                ...
                if(postId==""){
                    postNames = postNames.substring(0,postNames.length-1) + '<br/>' + postName;
                }else{
                    if(postNames.lastIndexOf("/") == postNames.length-1 ){
                        postNames = postNames.substring(0,postNames.lastIndexOf("/"));
                    }
...
function loadBusinessForm() {
	...
	url = url 
		...
		+ '&DTL_SESSION_ID=' + resultData.returnSessionId
		+ '&agentUrl=' + agentUrl
		+ '&userId=' + resultData.returnUserId;
