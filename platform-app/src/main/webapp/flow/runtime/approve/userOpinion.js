...
function resizeGrid(){
	$(".ui-jqgrid-bdiv table").jqGrid().setGridHeight($(window).height()-140);
...
function changeGridIntoEditStatus(tableId){
	...
	grid.find("tr").eq(0).nextAll().each(function (index) {
		$(this).attr("id", index);
		var sx=$(this);
		sx.find(".add-icon").attr('herf','javascript:addRow('+index+')');
		sx.find(".delete-icon").attr('href','javascript:deleteRow('+index+')');
		sx.find(".down-icon").attr('href','javascript:downRow('+index+')');
		sx.find(".up-icon").attr('href','javascript:upRow('+index+')');
		sx.find("input").attr({"rowid":index,"id":index+"_opinion"});
	});
...
function operateFormatter(cellvalue, options, rowObject){  //image/flow/icon_add.png
	...
	"<a  class='order-icon add-icon' href=\"javascript:addRow("+options.rowId+")\"  ></a>"+
	"<a class='order-icon delete-icon' href=\"javascript:deleteRow("+options.rowId+")\" ></a></div>";
}
...
function upRow(rowId){
	...
	var grid = $("#oneList");
	grid.jqGrid('saveRow',rowId);//关闭所有编辑模式
	var my = grid.jqGrid().getRowData(rowId);
	var newid = rowId-1;
	grid.jqGrid('delRowData', rowId);
	grid.jqGrid('addRowData', newid, my, 'before', newid);//插入行
}
/**
 * 向下按钮的点击事件
 * @param tableType
...
function downRow(rowId){
	var grid = $("#oneList");
	var len = grid.getGridParam().reccount;
	if(rowId == len -1){
		return false;
	}
	grid.jqGrid('saveRow',rowId);//关闭所有编辑模式
	...
	var newid = rowId+1;
	grid.jqGrid('delRowData', rowId);
	grid.jqGrid('addRowData', newid, my, 'after', rowId);//插入行
}
/**
 * 增加按钮的点击事件
 * @param tableType
...
function addRow(rowid){
	...
	// data.opinion = "";
	data.isDefault = "";
	grid.jqGrid('addRowData', newid, data, 'after', rowid);//插入行
}
/**
 * 删除按钮的点击事件
 * @param tableType
