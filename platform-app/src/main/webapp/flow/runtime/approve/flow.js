...
$(function () {
    ...
    //取消传阅
    $('#withdrawPassRead').on('click',function () {
    	var id=$('#passReadGrid').jqGrid('getGridParam','selrow');
    	var selectData = $('#passReadGrid').jqGrid('getRowData',id);
    	if(id){
    		if(!selectData.cancelPassReadTime){
    			$.ajax({
    	 		       url:hostUrl+"flow/instance/cancelPassAndRead/" + id,
//    	 		       data:JSON.stringify(),
    	 		       type:'POST',
    	 		       ...
    	 		       success:function (resultData) {
    	 		           if(resultData) {
    	 		               ...
    	 		               if(successFlag) {
    	 		            	   //刷新表格
    	 		            	   jQuery("#passReadGrid").trigger("reloadGrid");
    	 		               }else {
    ...
    var initInstanceData = function () {
        ...
        postData.taskId = urlParams.taskId;
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (data.success) {
                    ...
                    if(data.code=='BusinessType'){
                    	$.xljUtils.tip('red', '该流程实例尚未迁移完成,请联系系统管理员！');
                    }
    ...
    //流程通用ajax调用
    var commonAjaxAction = function (urlText, paramObject, actionName,tipFlag){
    ...
    var initOperationBtnEvent = function (btnObj,btnCode,instanceId) {
        switch(btnCode){
            ...
            case 'finishApproval':
                btnObj.on('click',function () {
                    $.xljUtils.confirm("blue", "确定要审结此流程吗？", function () {
                    	btnObj.css("background","#F0F0F0");
            ...
            case 'cancelInstance':
                btnObj.on('click',function () {
                    $.xljUtils.confirm("blue", "确定要作废此流程吗？", function () {
                    	btnObj.css("background","#F0F0F0");
                    	btnObj.attr({ disabled: "disabled" });
                        var urlText = "flow/instance/cancelInstance/" + instanceId;
                        var paramObject = {};
                        var commonAjaxDef = commonAjaxAction(urlText, paramObject, "作废流程",true);
                        $.when(commonAjaxDef).done(function (success) {
                            if(success){
                                window.location.reload();
                            }
           ...
           /* //修改审批人
            ...
            case 'withDrawFlow':
                btnObj.on('click',function () {
                    $.xljUtils.confirm("blue", "确定要撤回流程？", function () {
                        $.ajax({
                            type: 'GET',
            ...
            case 'withDrawTask':
                btnObj.on('click',function () {
                    ...
                    if($.xljUtils.getUrlParam("preTaskId") != null) {
                        url = url + '&taskId=' + $.xljUtils.getUrlParam("preTaskId");
                    }else{
            ...
            case 'print':
                btnObj.on('click',function () {
                    ...
                    // 如果是人事考勤类的打印，针对以下元素进行table的layout设置
                    // 宽度为1000， table-layout设定为fixed
                    // TODO: 这个样式的改动，应该在人力项目中进行设定
                    var $hrTable = $(window.frames["bizForm"].document).find('#sysApplyFrom table');
                    if (bizFormFrame && $hrTable.length > 0) {
                        $(bizFormFrame).css({'width': '100%'});
                        // $hrTable.css({'table-layout': 'fixed'});
                    }
                    ...
                    setTimeout(function () {
                        ...
                        $('.flow-tab').show();
                        $('body').css('width','100%');
                        // 如果是人事考勤类的打印，打印完毕后，宽度恢复
                        if (bizFormFrame && $hrTable.length > 0) {
                            $(bizFormFrame).css({'width': '100%'});
                            $('#approvalList').width('100%');
                            // $hrTable.css({'table-layout': 'fixed'});
                        }
    ...
    var drawApproveList = function (approvalTable, approvalList) {
        ...
        var theader = $('<tr class="form-tr form-header" >' +
            ...
            '<td style="width: 10%; text-align: left; font-weight: bold;">责任人<span class="groupChatIcon" onclick="onLine(this,null,\'group\');"></span></td>' +
        ...
        //将所有的postid放到一个数组中
        var allPostIdArr = [];
        var loginUserId = $("#loginUserId").val();//当前登录人
        if(approvalList){
            for (var i = 0; i < approvalList.length; i++) {
                ...
                var approverNameTd;
                if(obj.acType != '3' && approverId && approverId != loginUserId){
                    approverNameTd = $('<td style="width: 10%; text-align: left;">'+ approverName +"<span title=\"发起聊天\" class=\"startChartIcon\"   onclick=\"onLine(this,'"+ approverId +"','single');\" ></span></td>");
                    /*if(toUsersIds.indexOf(approverId) == -1){
                     toUsersIds.push(approverId);
                     }*/
                }else{
                    approverNameTd = $('<td style="width: 10%; text-align: left;">'+ approverName + "</td>");
                }
                ...
                if($.inArray(curPostId, postIdArr) != -1 && curPostId != prePostId){
                    ...
                    delete repeatPostIdNumJson[curPostId ];
                }
                ...
                if ((obj.taskStatus == '1' || obj.taskStatus == '' || obj.taskStatus == null) && obj.postStatus != '3') {
    ...
    var initOperation = function (operations,instanceId) {
        ...
        $.each(operations,function (i,operation) {
            if(operation){
                ...
                operArr.push(operation.defaultNote);
...
//            }
            if(checkDefaultOpinions(operArr)!=-1){
            	$('#approvalText').val($(this).data('defaultNote'));
            }
        });
    ...
    //校验审批意见是否是默认意见,如果是则根据操作切换相应的意见否则不切换
    var checkDefaultOpinions = function(arr){
//    	var arr = [ "通过", "协办", "转办", "打回", "回复", "接受", "不接受", "沟通发起人", "无异议" ]; 
    	if($.trim($('#approvalText').val())){
    		return $.inArray($('#approvalText').val(), arr);
    	}else{
    		return 1;
    	}
    }
    ...
    var submitProcess = function (flowInstance) {
    	var urlTaskFirst=false;//url上当前任务id如果是点击任务列表进入的审批界面,那么应该处理的是当前审批中的任务而不是默认的第一个任务
        var approvalSubmit = {};
        approvalSubmit.instanceId = flowInstance.instanceId;
        var currApproCount=0;//统计当前审批人的待审数量,不能放到下面的循环里
        $.each(flowInstance.list, function(index, item) {
            if(item.approverId == flowInstance.currentUserId && item.taskStatus == '2') {
            	if($.xljUtils.getUrlParam('taskId')==item.taskId){
            		urlTaskFirst=true;
            	}
            	currApproCount++;
            }
        ...
        /**
         * 如果当前审批人有多个待审（例：有多个人沟通发起人，此时发起人登录就有多个待审），但是审批人是点击不同的待办进入的审批界面，此时默认取第一个任务会出现问题
         */
        if(currApproCount>1 && urlTaskFirst){
        	approvalSubmit.taskId = $.xljUtils.getUrlParam('taskId');
            approvalSubmit.msgId = $.xljUtils.getUrlParam('msgId');
        }
...
//                          location.reload();
                            if(window.location.href.indexOf("preTaskId")!=-1){
                            	window.location.href = window.location.href.replace(new RegExp($.xljUtils.getUrlParam("preTaskId"),"gm"),approvalSubmit.taskId);
                            }else if(window.location.href.indexOf("&")!=-1){
                            	window.location.href = window.location.href+"&preTaskId="+approvalSubmit.taskId;
                            }else{
                            	window.location.href = window.location.href+"?preTaskId="+approvalSubmit.taskId;
                            }
        ...
        $.when(instanceDataDef).done(function (flowInstance) {
            if(flowInstance){
                if(flowInstance.operations != null && flowInstance.operations.length > 0 && $.xljUtils.getUrlParam("source") != 'FQ') {
                    ...
                    $('#approveArea').parent('div').show();
                    //聊天按钮隐藏
                    $(".not-approve-node").find("td").find("span").css("display","none");
                    //初始化过程附件
                    ...
                    //聊天按钮隐藏
                    $(".approval-list").find("td").find("span").css("display","none");
                }
            }
      ...
      $.when(instanceDataDef).done(function (flowInstance) {
          ...
          $('#passReadGrid').jqGrid( {
              ...
              colModel : [
				  ...
				  {name: 'transationDate',label : '传阅时间', align:"center" },
				  {name: 'cancelPassReadTime',label : '取消传阅时间', align:"center" },
				  {name: 'cancelPassReadUserName',label : '撤回人', align:"center" }
              ],
              //forceFit:true,                                      //当为ture时，调整列宽度不会改变表格的宽度。
              //width: '100%',
...
var onLine = function (Obj, toUserId,type) {
    ...
    var processTitle ="关于流程【" + $("#processTitle").val()+"】的会话";
