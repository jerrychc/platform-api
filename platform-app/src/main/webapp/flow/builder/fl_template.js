...
define(['xljUtils', 'utils','actions'], function (xljUtils, utils,action) {
    ...
    var getFlDataForSave = function (flBaseData) {
        ...
        if(flBaseData&&flBaseData.flData){
            ...
            flData.name = flData.name.replace(/\\/g,'').replace(/\'/g,'\\\'');
            flData.code = flData.code.replace(/\\/g,'').replace(/\'/g,'\\\'');
            flData.flowTitle = flData.flowTitle.replace(/\\/g,'').replace(/\'/g,'\\\'');
            flData.businessObjectName = flData.businessObjectName.replace(/\\/g,'').replace(/\'/g,'\\\'');
            ...
            for (var i = 0; i < childCells.length; i++) {
                ...
                if (obj.isEdge()) {
                    ...
                    stepObj.name = obj.value?obj.value.replace(/\'/g,'\\\''):obj.value;
                    ...
                    acObj.id = obj.id;
                    acObj.name = obj.value?obj.value.replace(/\'/g,'\\\''):obj.value;
                    acObj.flId = flId;
    ...
    var closeWin = function () {
        ...
        $.xljUtils.confirm('blue','请确保修改的模板数据已经保存！',function () {
            window.open('','_self');
            window.close();
        },function () {
            //窗口关闭时提醒
