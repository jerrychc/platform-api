...
$(function () {
    ...
    $('#overdueTime').on('keyup',function () {
       var overdueTimeVal = $(this).val();
        if(overdueTimeVal!=0&&overdueTimeVal!=''){
            $('#overdueHandle').removeAttr("disabled");

        }else{
            $('#overdueHandle').attr("disabled","disabled");
        }
    });
    ...
    var initFormData = function () {
        ...
        if("start"==nodeType||"end"==nodeType){
            $('#id').val(extra.id);
            $('#acType').val(extra.acType);
            $('#nodeId').val(extra.nodeId);
            $('#name').val(extra.name);
            $('#code').val(extra.code);
            ...
            for(var item in extra){
                ...
                if(item=='isStart' || item=='isAddLabel'){
                    ...
                    if($(inputObj[0]).attr('type')=='radio'){
                        ...
                        //if($(inputObj[0]).attr('name')=='approveTypeId'&&(!value))
                        if(value){
    ...
    window.saveAcFormData = function(){
        ...
        for(var i in formDataArray){
            ...
            if (name == 'isStart' || name == 'isAddLabel') {
                ...
                acBaseInfo[name]=formDataArray[i].value.replace(/\\/g,'');
            }
        }
        ...
        if(!acBaseInfo.isAddLabel&&acBaseInfo.acType=='2'){
            ...
            for (var i = 0; i < approvePartnerArr.length; i++) {
              ...
              //  if(obj.participantType!='4'&&(!participantName||participantName=='')
                ...
                if(obj.participantType!='4'&&(!participantName||participantName=='')  ){
                    if(obj.participantType!='1' && obj.participantScope != '12'){
