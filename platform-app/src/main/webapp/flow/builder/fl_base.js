        ...
        if (window.parent && $.isFunction(window.parent.getVariableListDef)) {
            ...
            $.when(variableListDef).done(function (variableList) {
                ...
                if (variableList) {
                    ...
                    $('#flowTitleShow').on('keyup',function () {
                        //alert(JSON.stringify(variableObjJson));
                        var flowTitleShow = $('#flowTitleShow').val();
                        var validFlowTitle = $('#flowTitleShow').val();
                        for(var item in variableObjJson){
                            var fieldName = variableObjJson[item];

                            flowTitleShow = flowTitleShow.replace('@'+fieldName+'@','@'+item+'@');
                            validFlowTitle = validFlowTitle.replace('@'+fieldName+'@','');

                        }
                        if(validFlowTitle.indexOf('@')>-1){
                            $.xljUtils.tip('blue','输入的流程标题格式不正确！请输入正确格式：@和@之间必须是流程变量，不能包含常量');
                        }else{
                            $('#flowTitle').val(flowTitleShow);
                        }
    ...
    var initForm = function () {
        if (window.parent && $.isFunction(window.parent.getFlDataDef)) {
            ...
            $.when(flDataDef).done(function (flData) {
                flData.flData.name = flData.flData.name?flData.flData.name.replace(/\\/g,''):'';
                flData.flData.code = flData.flData.code?flData.flData.code.replace(/\\/g,''):'';
                flData.flData.flowTitle = flData.flData.flowTitle?flData.flData.flowTitle.replace(/\\/g,''):'';
                flData.flData.businessObjectName = flData.flData.businessObjectName?flData.flData.businessObjectName.replace(/\\/g,''):'';
