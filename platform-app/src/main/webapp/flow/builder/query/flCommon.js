    ...
    $.when(participantNameDef).done(function (participantNameMap) {
        if(participantNameMap){
            for(var partnerId in participantNameMap){
                ...
                var id = participantIdInputObj.attr("id");
                id = id.replace(/participantId/,"participantScope");
                participantType = $("#"+id).val();
                if(participantType == "11"||participantType == "12"){
                    var prefixName = participantNameMap[partnerId];
                    var arr = prefixName.split("/");
                    var name = arr[arr.length-1];
                    var orgName = prefixName.substring(0,(prefixName.length-name.length-1));
                    participantIdInputObj.siblings(':input[name="participantName"]').attr("title",orgName);
                    participantIdInputObj.siblings(':input[name="participantName"]').val(name);
                }else{
...
var addStandardPostRow = function (table) {
    ...
    participantInputObj.find('.input-group-addon').xljSingleSelector({
        title: '选择标准岗位',
        selectorType: 'role',
        targetId: 'participantId_' + tableId + '_'  +  index,
        targetName: 'participantName_' + tableId + '_'  +  index,
        treeParam: {type: '1',roleCataStatus:true,rootDelFlag:false},
...
var addPersonRow = function (table) {
    ...
    participantInputObj.find('.input-group-addon').xljSingleSelector({
        ...
        targetId: 'participantId_'+ tableId + '_'  + index,
        targetName: 'participantName_'+ tableId + '_'  + index,
        treeParam: {rootStatus:true,orgStatus:true,orgDelFlag:false,userDelFlag:false},
        saveCallback: function (selectedData, ele) {
...
var addRoleRow = function (table) {
    ...
    participantInputObj.find('.input-group-addon').xljSingleSelector({
        ...
        targetId: 'participantId_' + tableId + '_' + index,
        targetName: 'participantName_' + tableId + '_' + index,
        treeParam: {type: false,roleCataStatus:true,rootDelFlag:false},
        saveCallback: function (selectedData, ele) {
            selectorCallback(selectedData,ele);
        }
...
var selectorCallback = function (selectedData,ele) {
    ...
    if(selectedData&&$.isArray(selectedData)){
        ...
        if(type=='1'){//人员
            //id = obj.id;
            id = !obj.userId||obj.userId==''?obj.id:obj.userId;
            if(obj.postName){
        ...
        if(type=='1' && !obj.postName){
            oldTrObj.find(':input[name="participantName"]').attr("title",obj.belongOrgName);
        }
    }
