...
function getDiv2HtmlOfRoleType(blockType, trSort, valueText, valueId, status){
    ...
    "<span class=\"input-group-addon w28 multiple-selector\" id='participantId_"+blockType+"_"+trSort+"_31' data-selectorType='role' data-treeParam='{\"type\":1,\"roleCataStatus\":true}'  data-savecallback=\"multiSelectorCallback\" data-title=\"选择标准岗位\">" +
...
function getDiv2HtmlOfCommonRole(blockType, trSort, valueText, valueId, status){
    ...
    "<input type='hidden' id='status_"+blockType+"_"+trSort+"' name='status_"+blockType+"_"+trSort+"' value='"+status+"'/>"+
    //"<span class=\"input-group-addon w28\" onclick=\"chooseOuterData('"+blockType+"','"+trSort+"','31')\" >" +
    "<span class=\"input-group-addon w28 multiple-selector\" id='participantId_"+blockType+"_"+trSort+"_51' data-selectorType='role' data-treeParam='{\"type\":0,\"roleCataStatus\":true}' data-savecallback=\"multiSelectorCallback\" >" +
    "<a href=\"javascript:void(0);\" class=\"fa fa-ellipsis-h\" aria-hidden=\"true\"></a></span></div>";
}
	...
	if(data.length>=1){
		...
		if(data && data.length>0){
			...
			for(var idx=0; idx<data.length; idx++){
				var nextItem = data[idx];
				if(busiType == "11" && !nextItem.postName){
					for(var s=0;s<tableDataList.length;s++){
						var row=tableDataList[s];
						if(row.participantType=="1" && row.participantId.indexOf(nextItem.id)!=-1){
							showTextId = "participantIdName_"+blockType+"_"+row.sort;
							$("#"+showTextId).val(nextItem.name);
			                $("#"+showTextId).attr("title",nextItem.belongOrgName);
						}
					}
				}
			}
		}
//		if(busiType == "11" && !firstItem.postName){
//            $("#"+showTextId).attr("title",firstItem.belongOrgName);
//        }
		//showResultTextToLabel(blockType);
	}
} 
