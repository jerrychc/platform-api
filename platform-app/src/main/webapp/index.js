...
//    $('.menu_search').css({display:"none"});
    ...
    $('#reLoginBtn').on('click', function () {
        ...
        $.ajax({
            ...
            success: function (json) {
                if (json.success) {
                    //业务系统登出地址
                    var bSysCASLogoutUrl = json.result;
                    if(bSysCASLogoutUrl){
                        $.ajax({
                            type: 'get',
                            url: bSysCASLogoutUrl,
                            dataType: 'jsonp',
                            contentType: 'application/json',
                            data: "{}",
                            success: function () {
     ...
     * 系统弹出窗口添加事件
     */
    $('#proManage').on('shown.bs.modal', function () {
        var dom = $('.project-item > li').find("[procode='BI']");
        if(dom.length){
            var prourl = dom[0].getAttribute("prourl");
            if(prourl.length>0&&prourl.indexOf("&qr=")>0){
                var timeKey  = setInterval(function(){
                    $.post(hostUrl+"sys/authentication/sendUrl?_time="+new Date().getTime(),{prourl:prourl},function(result){
                        if(result.success){
                            if(result.msg){
                                var key = result.msg;
                                dom[0].setAttribute("prourl",prourl.substring(0,prourl.indexOf("&qr=")+4) +key);
                                dom[0].children[0].setAttribute("src","data:image/jpeg;base64,"+result.result);
                            }else{
                                if(prourl.substring(0,prourl.indexOf("&qr="))==result.result){
                                    initBIProject('BI');
                                }else{
                                    initBIProject('DEMOBI');
                                }
                            }
                        }
                    });
                },5000);
                dom.attr("timeKey",timeKey);
            }
        }
    });
     ...
     * 初始化所有系统
     */
    function initBIProject(sysCode) {
                ...
                if (data.success) {
                    ...
                    for (var i = 0; i < projectItems.length; i++) {
                        ...
                        if (proIcon && proIcon != '') {
                            ...
                            iconObj='<img src="data:image/jpeg;base64,'+proIcon+'" style="width:80px;height:80px;margin-top:10px;margin-bottom:7px">';
                        }
                        aObj.append(iconObj);
                        var proNamePObj = $('<p></p>');
                        ...
                        aObj.on('click', function () {
                            ...
                            //BI系统特殊处理
                            if(proCode=='BI'){
                                proUrl = proUrl.substring(0,proUrl.indexOf("&qr="));
                            }
                            //根据系统ID初始化系统下一级和二级菜单
                        ...
                        $('.project-item ').find('a[procode="DEMOBI"]').parent('li').hide();
                        if (proCode==sysCode) {
                            firstProA = aObj;
                        }
                    }
     ...
     * 系统关闭窗口删除事件
     */
    $('#proManage').on('hidden.bs.modal', function () {
        var dom = $('.project-item > li').find("[procode='BI']");
        if(dom.length){
            var timeKey = dom[0].getAttribute("timeKey");
            if(timeKey){
     ...
     * 扫码跳转页面事件
     */
    function bindQREvent(e,url){
        var proOpenmode = $(e).attr('proopenmode');
        var proCode = $(e).attr('procode');
        var proName = $(e).attr('proname');
        var proUrl = url;
        var proId = $(e).attr('proid');
        //BI系统特殊处理
        if(proCode=='BI'){
            proUrl = proUrl.substring(0,proUrl.indexOf("&qr="));
        }
                    ...
                    for (var i = 0; i < projectItems.length; i++) {
                        ...
                        if (proIcon && proIcon != '') {
                            ...
                            if(proCode=='BI'){
                                ...
                                iconObj='<img src="data:image/jpeg;base64,'+proIcon+'" style="width:60px;height:60px;margin: 22px 34px 12px 34px;">';
                            }
                        }
                        aObj.append(iconObj);
                        var proNamePObj = $('<p></p>');
                                    ...
                                    if(treeNode.linkType=='INNER'){
                                        ...
                                        if(menuUrl){
                                            ...
                                            if(menuUrlObj){
                                                ...
                                                if(!_menuCode || _menuCode=='undefined'){
                                                    _menuCode = null;
                                                }
    ...
    $('#favoriteModal').on('shown.bs.modal',function () {
        ...
        if(resourceLink.indexOf('?')!=-1){
            ...
            if(resourceLink.indexOf('_menuCode')==-1&&$('body').data('_menuCode')){
