 ...
 */
function updateSort() {
    var obj = dataPointGrid.jqGrid("getRowData");
    if(obj==null ||obj.length==0){
        pop_tip_open("blue","当前无数据可修改排序号");
        return false;
    }
    jQuery(obj).each(function(){
        dataPointGrid.jqGrid('editRow', this.id);
    });
    $("input[name='sort']").blur(function(){
        var sortVal=$("#"+this.id).val();
        if(!sortVal){
            flag = true;
        	return;
		}
        if(  !(/^(\+|-)?\d+$/.test( sortVal )) || sortVal <=0 || sortVal.length >15){
            pop_tip_open("blue","排序号只能输入正整数，并且长度小于15");
            $(this).focus();
            ffId = this.id;
            if(userAgent.indexOf("Firefox") > -1){
                window.setTimeout (function(){ document.getElementById(ffId).focus();},0 );
            }
            flag = false;
            return false;
        }else{
...
function saveSort() {
    ...
    jQuery(obj).each(function(){
        ...
        if( !sortVal){
            ...
            if((/^(\+|-)?\d+$/.test( sortVal ))&& sortVal >0 && sortVal.length <=15){
				...
				if(haveSort.indexOf(sortVal)>=0){
                    res=2;
                    return false;
				}else{
					haveSort.push(sortVal)
				}
            }else{
                res = 1;
                return false;
    ...
    if(res==2){
        pop_tip_open("blue","同一业务对象下，排序号不允许重复");
        return false;
    }
    jQuery(obj).each(function(){
        dataPointGrid.jqGrid('saveRow', this.id);
    });
    var obj1 = dataPointGrid.jqGrid("getRowData");
    ...
    var urlBody = "sys/res/dataPoint/savePointSort";
