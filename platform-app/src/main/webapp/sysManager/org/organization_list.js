 ...
 * @param result
 */
function checkGroupBranch(treeNode,result){
    result = true;
    if (treeNode.isParent) {
        var childrenNodes = treeNode.children;
        if (childrenNodes) {
            for (var i = 0; i < childrenNodes.length && result; i++) {
                if(childrenNodes[i].type == "group" || childrenNodes[i].type == "branch"){
                    result = false;
                    return result;
                }else{
                    result = checkGroupBranch(childrenNodes[i], result);
                }
            }
        }
    }
...
//	console.log(nodes[0].children);
    var flag = checkGroupBranch(nodes[0]);
    if(!flag){
        pop_tip_open("blue","下级存在项目分期，不可删除");
        return false;
    }
    if(nodes.length != 1 ){
        ...
        return false;
    }else if(nodes[0].type == 'group'||nodes[0].type == 'branch'){
        pop_tip_open("blue","项目分期不可删除");
        return false;
    }else{
...
//	console.log(nodes[0].children);
    if(nodes.length != 1 ){
        ...
        if(nodes[0].type == 'group'||nodes[0].type == 'branch'){
            pop_tip_open("blue","项目分期不可进行禁用启用");
            return false;
        }
        var data={
            orgId:nodes[0].id,
...
function setDefaultPost(){
    var rowIds=jqGridUserPost.jqGrid('getGridParam','selarrrow');
    if(!rowIds || rowIds.length != 1){
        pop_tip_open("blue","请选择一条岗位");
    }else{
        var rowId = rowIds[0]
...
function showRenameBtn(treeId, treeNode) {
    if(treeNode.type == "cata" || treeNode.type == "group"|| treeNode.type == "branch"){
        return false;
    }else{
        return true;
    }
...
function beforeDrag(treeId, treeNodes) {
    ...
    if(treeNodes[0].type == 'group'||treeNodes[0].type == 'branch'){
        pop_tip_open("blue","项目分期不可拖拽");
        return false;
    }
...
function deleteUser(){
    ...
    if(null == ids){
        ...
        if(list.length == 0){
            ...
            pop_text_open("blue",'确定要移除这【'+ list.length +'】条用户吗？',function(){
                var deletedata ={
                    userIds:list,
                    postIds:post_ids,
                    type:"users"
...
function deletePost(){
    ...
    if(null == ids){
        ...
        if(list.length == 0){
            ...
            pop_text_open("blue",'确定要移除这【'+ids.length+'】条岗位吗？',function(){
                var deletedata ={
                    userIds:user_ids,
                    postIds:list,
                    type:"posts"
...
function editUserInfo(sign){
    if(sign == 1){
        editUserType = 1;
        var ids=jqGridUser.jqGrid('getGridParam','selarrrow');
        if(!ids || ids.length !=1){
            pop_tip_open("blue","请选择一个用户");
        }else{
            edit_userId=ids;
            window.open('user_edit.html');
        }
...
function updatestatus(status){
    ...
    if(ids == "" || ids == null){
        ...
        var uBody = "sys/org/user/lockUsersOrNot";
        var uAll = hostUrl + uBody;
        $.ajax({
            type:'PUT',
            url:uAll,
        ...
        pop_text_open("blue","确定要复制【"+copyNode.name+"】到【"+pasteNode.name+"】吗？",function(){
            ...
            $.ajax({
                ...
                success: function(json) {
                    if(json.success == true){
                        var treeObj=$.fn.zTree.getZTreeObj("treeDemo");
                        pasteNode = noGroupAndBranch(pasteNode);
                        //复制节点
...
function noGroupAndBranch(node){
    if(node.children.length > 0) {
        var arr=node.children;
        for(var i=0;i<arr.length;i++){
            if(arr[i].type=="group"||arr[i].type=="branch"){
                arr.splice(i,1);
                i--;
            }else{
                noGroupAndBranch(arr[i]);
            }
...
function updataCopyNode(data,node) {
    if(node.type == 'group' || node.type == 'branch'){
...
function importOrgs() {
    ...
    if(exampleType =="org"){
        var nds = zTreeObj.getSelectedNodes();
        if(nds.length < 1 ){
            pop_tip_open("blue","请先选择需要导入组织的节点");
            $(".loading").hide();
            return false;
        }else {
            var pNode = nds[0];
            if (!(pNode.type == "zb" || pNode.type == "company" || pNode.type == "dept")) {
                pop_tip_open("blue", "只有公司和部门可以进行导入");
                $(".loading").hide();
                return false;
            }
    ...
    if (!$("#orgExcelFile").val()) {
        pop_tip_open("blue", "请选择导入文件");
        $(".loading").hide();
        return false;
    }
    $('#addImport').modal('hide');
    var urlBody =exampleType=="org" ? "sys/org/orgnazation/readExcelForOrg" : "sys/org/user/readExcelForUser";
    var urlAll = hostUrl + urlBody;
    ...
    $.ajax({
        ...
        processData:false,
        contentType:false,
        data:new FormData($( "#excelOrgFrom")[0]),
        success: function(json) {
            resetFile("orgExcelFile");
            $(".loading").hide();
            if(json.success == true){
                var insertCount = json.result.insertCount;
                if(exampleType =="org" && insertCount <= 200){
                    var rightNodes = json.result.right;
                    recursionArray(rightNodes);
                    var cnode,pIndex,cLevel;
                    var pids = [];
                    var tempNodes = [];
                    for(var i in rightNodes){
                        pids.push(rightNodes[i].id);
                    }
                    var i = 0,j=0;
                    var length = rightNodes.length;
                    while( length > tempNodes.length){
                        cnode = rightNodes[i];
                        pIndex = pids.indexOf(cnode.parentId);
                        if(pIndex == -1 ){
                            cnode = zTreeObj.addNodes(pNode, cnode);
                            tempNodes.push(cnode[0]);
                        }else{
                            var _pNode = checkParentNode(tempNodes,cnode.parentId);
                            if(_pNode){
                                cnode = zTreeObj.addNodes(_pNode, cnode);
                                tempNodes.push(cnode[0]);
                            }else{
                                rightNodes.push(cnode);
                            }
                        }
                        i++;
                    }
                }
                showNote(json.result,exampleType);
            }else{
                pop_tip_open("red",json.msg);
            }
        },error:function(XMLHttpRequest, textStatus, errorThrown){
            $(".loading").hide();
            $('#addImport').modal('hide');
            pop_tip_open("red","导入失败");
        }
    })

}
function checkParentNode(tempNodes,parentId){
    var _pNode;
    for(var i in tempNodes){
        if(tempNodes[i].id == parentId){
            _pNode = tempNodes[i];
        }
    }
    return _pNode;
 ...
 * @param data
 */
function showNote(data,exampleType) {
    var insertCount = data.insertCount;
    var note = "成功导入" + insertCount + "条数据。";
    if(exampleType == "org"){
        var wrongCode = data.wrongCode;
        var wrongName = data.wrongName;
        var wrongParent = data.wrongParent;
        if(insertCount>200){
            note+= "您导入组织机构过多，请重新刷新组织结构树！</br>";
        }
        if(wrongCode.length + wrongName.length + wrongParent.length >0){
            note+= "</br>导入失败详情：</br>";
        }
        if(wrongCode.length >0 ){
            note+= "&nbsp;>>编码格式错误或编码重复：</br>";
            var wCode ;
            for(var i in wrongCode){
                wCode = wrongCode[i];
                note += "编码 : " + wCode.code + " ；名称 : " + wCode.name +"</br>";
            }
        }
        if(wrongName.length >0 ){
            note+= "&nbsp;>>字段格式错误（必输项为空，或长度不合法）：</br>";
            var wName ;
            for(var i in wrongName){
                wName = wrongName[i];
                note += "编码 : " + wName.code + " ；名称 : " + wName.name +"</br>";
            }
        }
        if(wrongParent.length >0 ){
            note+= "&nbsp;>>父节点不存在：</br>";
            var wParent ;
            for(var i in wrongParent){
                wParent = wrongParent[i];
                note += "编码 : " + wParent.code + " ；名称 : " + wParent.name +"</br>";
            }
        }
    }else{
        var wrongBlank = data.wrongBlank;
        var wrongLoginName = data.wrongLoginName;
        var wrongBelongOrg = data.wrongBelongOrg;
        var wrongCheckList = data.wrongCheck;
        if(wrongBlank.length + wrongLoginName.length + wrongBelongOrg.length + wrongCheckList.length >0){
            note+= "导入失败详情：</br>";
        }
        if(wrongBlank.length >0 ){
            note+= "&nbsp;>>必输项（用户名、账号、密码、所属组织、手机、邮箱）为空：</br>";
            var wCode ;
            for(var i in wrongBlank){
                wCode = wrongBlank[i];
                note += "用户名 : " + wCode.realName + " ；账号 : " + wCode.loginName + " ；所属组织 : " + wCode.belongOrgId + " ；手机号 : " + wCode.mobile + " ；邮箱 : " + wCode.email+"</br>";
            }
        }
        if(wrongCheckList.length >0 ){
            note+= "&nbsp;>>输入项不合法（字段长度不在范围内，或手机、邮箱不符合校验规则）：</br>";
            var wCode ;
            for(var i in wrongCheckList){
                wCode = wrongCheckList[i];
                note += "用户名 : " + wCode.realName + " ；账号 : " + wCode.loginName + " ；所属组织 : " + wCode.belongOrgId + " ；手机号 : " + wCode.mobile + " ；邮箱 : " + wCode.email+"</br>";
            }
        }
        if(wrongLoginName.length >0 ){
            note+= "&nbsp;>>账号重复：</br>";
            var wName ;
            for(var i in wrongLoginName){
                wName = wrongLoginName[i];
                note += "用户名 : " + wName.realName + " ；账号 : " + wName.loginName + " ；所属组织 : " + wName.belongOrgId + " ；手机号 : " + wName.mobile + " ；邮箱 : " + wName.email+"</br>";
            }
        }
        if(wrongBelongOrg.length >0 ){
            note+= "&nbsp;>>用户所属组织不存在或所属组织存在多条数据：</br>";
            var wParent ;
            for(var i in wrongBelongOrg){
                wParent = wrongBelongOrg[i];
                note += "用户名 : " + wParent.realName + " ；账号 : " + wParent.loginName + " ；所属组织 : " + wParent.belongOrgId + " ；手机号 : " + wParent.mobile + " ；邮箱 : " + wParent.email+"</br>";
            }
        }
    }

    $('#showContentModal').find('.modal-body').html(note);
...
function downExample() {
    var exampleType = $("#exampleType").val();
    var fileName = exampleType=="org" ?"导入组织模板.xlsx":"导入人员模板.xlsx";
    var urlBody = "sys/org/orgnazation/download?fileName="+fileName;
    var urlAll = hostUrl + urlBody;
    window.open(encodeURI(urlAll));
}
//---------------------------------------------
