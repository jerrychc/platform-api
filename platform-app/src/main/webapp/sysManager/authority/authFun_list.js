...
function treeNodeAddChildren(treeNode) {
    $("#roleList").append("<li><input type='checkbox' name='roleCheck' value='"+treeNode.id+"#"+treeNode.name+"'><span title='"+treeNode.name+"'>"+treeNode.name+"</span></li>");
    if (treeNode.children && treeNode.children.length>0){
        var childrenNodes = treeNode.children;
        for (var i=0;i<childrenNodes.length;i++){
            var childNode = childrenNodes[i];
            treeNodeAddChildren(childNode);
        }
    }
}
/**
...
function zTreeShowOnClick(event, treeId, treeNode) {
    ...
    if(objTypeShow == "post" && objTypeShow != treeNode.type){
        pop_tip_open("blue","请选择岗位");
        return false;
    }
    if(objTypeShow == "user" && objTypeShow !=treeNode.type){
	...
	$('#powerMenu').height($(window).height()-$('#powerApp').height() - 75);
...
function buttonzTreeOnClick(event, treeId, treeNode) {
	...
	colRoleModeldata.push({name : 'name',label : '对象',width : 370,align : "left",cellattr: addCellAttr});
...
function getRoleTree(objType) {
	...
	if("standardPost"==objType){//标准岗位
		...
		// urlBody = 'sys/org/roleUser/selectUserOrgTree';
		data = {rootDelFlag: 0, orgDelFlag: 0, postDelFlag: 0, rootStatus: 1,   postStatus: 1, userDelFlag:0};
	}else if("org"==objType){//组织
		urlBody = 'sys/org/root/getOrgTreeByType';
		data = {"rootDelFlag":0,"orgDelFlag":0,"type":"all"};
	}
...
//	        							var appids = appList[k].prefix_id.split("/");
                                    ...
                                    var type = $('#objectTypeButton option:selected') .val();
									for(var ao =0;ao<authorizationCheck_obj.length;ao++){
										...
										if(checkappIds[1] == appList[k].operation_id){
                                            if(type == 'org'){
                                                if(appList[k].prefix_id == checkappIds[0] ){
...
//	}
	var postData ={
        objectType : $('#objectType option:selected') .val(),
		saveData:authData
	}
...
//	}
	var postData ={
        objectType : $('#objectTypeButton option:selected') .val(),
	...
	if(operationId == "operationId"){
		if(e.checked){
			...
			for(var i=0;i<sameIds.length;i++){
				...
				if(saveId != e.id){
					var splitids = $("#"+saveId)[0].value.split("#");
					var splitType = splitids[3];
					if(splitType == "role" || splitType=="user" || splitType=="post" || splitType == "zb"|| splitType=="company"|| splitType=="dept"|| splitType=="group"|| splitType=="branch"){
            ...
            if(type == "zb"|| type=="company"|| type=="dept"|| type=="group"|| type=="branch"){
                return;
            }
...
//		console.log(e.checked);
		...
		if(e.checked){
			//把自己添加到缓存数据中
			if(type=="role"  || type=="user" || type=="post" || type == "zb"|| type=="company"|| type=="dept"|| type=="group"|| type=="branch"){
