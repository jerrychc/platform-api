...
function zTreeOnDblClick(event, treeId, treeNode) {
    ...
    if(dataRow.type == '2'){
        dataRow.scopeName = treeNode.name;
        dataRow.belongOrgName = treeNode.belongOrgName;
    }
	//将新添加的行插入到第一列  
    ...
    var jqGrid2 = jQuery("#listScope").jqGrid(
        {
			...
			gridComplete: function() {//当表格所有数据都加载完成，
                $("#listScope > tbody tr").each(function(){
                    var type = $(this).find("td[aria-describedby='listScope_type']").html();
                    if(type == "2"){
                        var orgName = $(this).find("td[aria-describedby='listScope_belongOrgName']").html();
                        $(this).find("td[aria-describedby='listScope_scopeName']").attr("title",orgName)
                    }
                });
				$.xljUtils.addGridScroll();
				$.xljUtils.gridResizeFn();
...
function getUserTree() {
    ...
    var ajaxData = {"userStatus":true,"userDelFlag":false};
    $.ajax({
        type:'POST',
        url:urlAll,
        dataType:'json',
