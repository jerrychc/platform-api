    ...
    jqGridRefScope = jQuery("#listRefScope").jqGrid({
        ...
        gridComplete: function () {
            ...
            for(var i = 0;i<objs.length;i++){
                var rowdata = objs[i];
                if(rowdata.type == "2"){
                    var arr = rowdata.scopeName.split("/");
                    var userName = arr[arr.length-1];
                    var orgName = rowdata.scopeName.substring(0,(rowdata.scopeName.length-userName.length-1));
                    $("#"+rowdata.id).find("td[aria-describedby='listRefScope_scopeName']").html(userName);
                    $("#"+rowdata.id).find("td[aria-describedby='listRefScope_scopeName']").attr("title",orgName);
                }
            }
            if(refScopeOnId){
...
function deleteRef(){
    ...
    if(!ids || ids.length==0){
        ...
        pop_text_open("blue",'确定删除这【'+ids.length+'】条数据吗？',function(){
            ids = ids.toString();
            var uBody = "sys/org/roleUser/deletePseudoBatch/"+ids;
            var uAll = hostUrl + uBody;
            $.ajax({
                type:'DELETE',
                url:uAll,
...
function deleteScope(){
    ...
    if(!ids || ids.length==0){
        ...
        if(list.length == 0){
            ...
            pop_text_open("blue",'确定要移除这条【'+list.length+'】管辖范围吗？',function(){
                var deletedata ={
                    userIds:list,
                    postIds:post_ids,
                    type:"users"
                }
                var uBody = "sys/org/roleUserPostScope/deleteBatch/"+ids;
                var uAll = hostUrl + uBody;
                $.ajax({
                    type:'DELETE',
                    url:uAll,
