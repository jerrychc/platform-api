...
var flowCode = $.getUrlParam("flowCode");
...
var loadingShowStyle = function(text){
...
var loadingHideStyle = function(){
	$("body").css("overflow","auto");
	$(".loading").hide();
};
//获取详情
function queryFormDataByMsgId() {
	var paramData = {msgId: msgId,instanceId:instanceId,businessId:businessId,appCode:appCode,flowCode:flowCode,appId:appId};
	var sessionSign = 0;
	$.ajax({
		type: "POST",
...
//                	str = "<p><span>"+dataList[i].name+":&nbsp;</span>  <span>"+valueText+"</span></p>";
			...
			if (flowList) {
				...
				for (var i = 0; i<flowList.length; i++) {
					...
					/*if(flowList[i].approverId !==null){
                            if(arrLst.indexOf(flowList[i].approverId)==-1){
                                arrLst.push(flowList[i].approverId)
							}
					}*/
                    strTitle=flowList[0].instanceName;
                    strTitle="关于流程【"+strTitle+"】的会话\n";
			...
			/*未审批人员单聊按钮隐藏*/
            if($(".dian_waiting").children().children().children().children().children().children().children("img").attr("src")){
                $(".dian_waiting").children().children().children().children().children().children().children("img").parent().hide();
            }

            //根据标签class名取值放入数组中，发起群聊
            var pass=$(".dian_pass")
            for(var i=0;i<pass.length;i++){
            	var arr=$(pass[i]).children().children().children().children().children().children().children("img").attr("data-userid");
                if(arr !==undefined){
                    if(arrLst.indexOf(arr)==-1){
                        arrLst.push(arr)
                    }
				}
            }
            ...
            var current=$(".dian_current")
            for(var i=0;i<current.length;i++){
            	var arr=$(current[i]).children().children().children().children().children().children().children("img").attr("data-userid");
                if(arr !==undefined) {
                    if (arrLst.indexOf(arr) == -1) {
                        arrLst.push(arr)
                    }
            ...
            if( dataDto.nextList &&  dataDto.nextList.length>0){
                $("#spcz").show();
...
function businessAccessoriesView(url,name,type) {
	// businessAccessoriesView("http://jzyerp.xyre.com:8084/qu/attachment/download?attachmentId=4342","基础插筋定位验收确认表.pdf",null);
	if("url"==type){
		window.location.href=url;
	}else {
		var obj = {};
		...
		obj.FILENAME = name;
			$.ajax({
				url: "/platform-app/univ/attachment/attachment/docConverterByBusiness" + "?time=" + Math.random(),
				data: JSON.stringify(obj),
				type: "POST",
				contentType: 'application/json',
				dataType: 'JSON',
...
setupWebViewJavascriptBridge(function(bridge) {
    document.getElementById("groupChat").onclick = function(e) {
        ...
        //js调用objc
        bridge.callHandler(
            'jzyJsHandlers', {'method':3,'params':{'title': strTitle,'thirdUserIdList':arrLst}},
            function(response){
    ...
    $(document).on("click",".chat_phone_zx",function(e){
        ...
        bridge.callHandler(
            'jzyJsHandlers',
            {'method':4,'params':{'title': strTitle,'thirdUserId':$(this).attr("data-userid")}},
            function(response){
                log('此处不用处理，可为空',response);
            }
        );
