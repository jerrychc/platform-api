...
$.ajax({
    type: "get",
    url:'/platform-app/sys/base/customForm/getSessionId',
    data:'',
    dataType: "json",
...
var showListFragment = function (J, C) {
    ...
    for (var E = 0; E < J.length; E++) {
        ...
        if(F.operationTip){
            ...
            var remainderNames="";
            try {
                var operationTipData = JSON.parse(F.operationTip);
                var strReplyNames=operationTipData.replyNames;
                var strRemainderNames=operationTipData.remainderNames;

                //已回复人员
                if(strReplyNames==undefined){
                    strReplyNames="";
                }else if(strReplyNames.indexOf("[") !==-1){
                    strReplyNames=strReplyNames.substring(1);
                    strReplyNames=strReplyNames.substring(0,strReplyNames.length - 1);
                };

                //未回复人员
                if(strRemainderNames==undefined){
                    strRemainderNames="";
                }else if(strRemainderNames.indexOf("[") !==-1){
                    strRemainderNames=strRemainderNames.substring(1);
                    strRemainderNames=strRemainderNames.substring(0,strRemainderNames.length - 1);
                }



                if(operationTipData.remainder>0){
                    imgSpan="<img class='xb' style='width: 22px;' src='../myimg/xb_no@2x.png'/>";
                    replyNames="<span class='spanWord'>协办已回复人员："+strReplyNames+"</span>";
                    remainderNames="<span class='spanWord'>协办未回复人员："+strRemainderNames+"</span>";
                }else{
                    imgSpan="<img class='xb' style='width: 22px;' src='../myimg/xb_yes@2x.png'/>";
                    replyNames="<span class='spanWord'>协办已回复人员："+strReplyNames+"</span>";
                    remainderNames="<span class='spanWord'>协办未回复人员："+strRemainderNames+"</span>";
                }
            } catch (e) {
        ...
        var K;
        if (C == 0 || C == 1) {
            K = "<div style='display: inline-block; width: 94%; margin: 0 auto;'><h4 class='mui-ellipsis-2' style='width: 94%; word-break: break-all;word-wrap: break-word;font-weight: normal'>" + F.title + "</h4></div><span class='imgSpan'>"+imgSpan+"</span>";
            if (A != undefined) {
            ...
            K = "<h4 class='mui-ellipsis-2' style='display: inline-block; width: 94%; word-break: break-all;word-wrap: break-word;font-weight: normal'>" + F.title + "</h4><span class='imgSpan'>"+imgSpan+"</span>"
            K+= replyNames;
            K+= remainderNames;
        }
