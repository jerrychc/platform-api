...
//	$('#registerId').val(registerId);
        ...
        success: function(data) {
        	if(data.success){
        		 if(data.result){	
        			 ...
        			 $("#appSystem").append("<option value=''>请选择</option>")
        			 for(var o in company){
        				 $("#appSystem").append("<option value='"+company[o].code+"'>"+company[o].name+"</option>");
        				 appnameStr += company[o].code+":"+company[o].name+";";
        			 }
        		 }
        	}else{
	...
	jQuery("#systemGrid").jqGrid(
            {
                url: hostUrl+'finance/accountSet/page',
                ...
                rownumbers:true,
                multiselect:true,
                postData:{registerId:registerId,appCode:appCode,delflag:0},
                jsonReader : {
                    repeatitems: false
                },
