...
/*	$.xljUtils.resizeNestedGrid();*/
	...
	$('#assNames,#selectAssNames').on('click', function() {
		...
		var accountSetId=$("#selectCompany").val();
		var data={accountSetId:accountSetId,delflag:0};
		$.ajax({
			type: "post",
	...
	$('#bizItemNames,#SelectbizItemNames').on('click', function() {
		...
		$("#myBizItemNames").modal("show");
		$("#myBizItemNames .modal-dialog").height($(window).height()*0.8);
		$("#myBizItemNames .modal-dialog .ztree-box").height($("#myBizItemNames .modal-dialog").height()-130);
		$("#myBizItemNames .modal-dialog .tableStyle").height($("#myBizItemNames .modal-dialog").height()-130);
		...
		$.ajax({
			type: "post",
			url: hostUrl + "finance/sysBizItem/getSysBizItemList",
			dataType: "json",
			...
			success: function (typeNodes) {
				...
				recursionArray(zNodes);
				var treeObj = $.fn.zTree.init($("#_bizItemNameszTree"), setting, zNodes);
//				treeObj.expandAll(true);
				//
				$.xljUtils.addTreeScroll();
				$.xljUtils.treeResizeFn();
				//默认加载第一个菜单的列表数据
				var nodesId=bizidForList.split(",");
				debugger
				for(var o in nodesId){
					for(var j in zNodes){
						if(zNodes[j].id==nodesId[o]){
							var row=$("<tr style='text-align:center'>" +
									"<td style='width:5%;'></td>" +
									"<td  style='display:none'>"+zNodes[j].id+"</td>" +
									"<td style='width:19%;'>"+zNodes[j].name+"</td>" +
									"</tr>");
								row.on("dblclick",function(){
								$(this).remove();
								bizidForList=bizidForList.replace(treeNode.id,"");
							});
							$("#bizItemNamesList").append(row);
							resetBizModelNum();
						}
					}
				};
...
function menuCallback(treeNodes,e) {
	...
	$("#accountCaptionForm").find("input[id='bizItemIds']").val(treeIds.substring(0,treeIds.lastIndexOf(",")));
	$("#accountCaptionForm").find("input[id='bizItemNames']").val(treeNames.substring(0,treeNames.lastIndexOf(",")));
}
...
function appendDataToRightBiz(e,treeId,treeNode){
	var row=$("<tr style='text-align:center'>" +
			"<td style='width:5%;'></td>" +
			"<td  style='display:none'>"+treeNode.id+"</td>" +
			"<td style='width:19%;'>"+treeNode.name+"</td>" +
			"</tr>");
	row.on("dblclick",function(){
		$(this).remove();
		bizidForList=bizidForList.replace(treeNode.id,"");
	});
	if(bizidForList.indexOf(treeNode.id)<0){
		$("#bizItemNamesList").append(row);
	}
	bizidForList+=treeNode.id;
	resetBizModelNum();
}

function myBizItemNames(e,treeId,treeNode){
	var row=$("<tr style='text-align:center'>" +
			"<td style='width:5%;'></td>" +
			"<td  style='display:none'>"+treeNode.id+"</td>" +
			"<td style='width:19%;'>"+treeNode.name+"</td>" +
...
function resetBizModelNum(){
	$("#bizItemNamesList").find("tr").each(function(i){
		if(i>-1){
			$(this).find("td").eq(0).html(i+1);
		}
	});
...
function saveBizItem(){
	var idlist='';
	var nameList='';
	$("#bizItemNamesList").find("tr").each(function(i){
		idlist+=$(this).find("td").eq(1).text()+",";
		var names = $(this).find("td").eq(2).text().split(" ");
		if(names.length>1){
			nameList+=$(this).find("td").eq(2).text().split(" ")[1]+",";
		}else{
			nameList+=$(this).find("td").eq(2).text().split(" ")[0]+",";
		}
		
	})
	$("#bizItemIds").val(idlist.substring(0,idlist.lastIndexOf(",")));
	$("#bizItemNames").val(nameList.substring(0,nameList.lastIndexOf(",")));
	$("#myBizItemNames").modal("hide");
	bizidForList = "";
}

function clearBizItem(){
	$("#bizItemNamesList").html("");
	if($("#bizItemIds").val()!=null){
		bizidForList = $("#bizItemIds").val();
	}else{
		bizidForList = "";
	}
	...
	$.ajax({
		...
		success : function(data) {
			if (data.success) {
				...
				debugger
				if(result.bizItemIds!=null){
					bizidForList=result.bizItemIds;
				}
